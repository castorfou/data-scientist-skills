...
def saveFromFileIO(dict_urls, prefix='', proxy='', prefixToc='', forceDownload=False):
    if (len(prefixToc)>0):
        prefix=getPrefixfromTOC(prefixToc)
    hash =  Simhash(dict_urls).value
    filename_lock = prefix+str(hash)+".lock"
    #si le fichier existe
    if (forceDownload and os.path.exists(filename_lock)):
        os.remove(filename_lock)
        print("Téléchargements déjà effectués - FORCE")
    if (os.path.exists(filename_lock)):
        ...
        for python_type, filename_url in dict_urls.items():
            for filename, url in filename_url.items():
                ...
                #print('#debug', ['curl', curl_proxy_option, url, '--output',prefix+filename])
                out = subprocess.check_output(['curl', curl_proxy_option, url, '--output',prefix+filename],stderr=subprocess.STDOUT)
...
def getPrefixfromTOC(prefixToc):
    ...
    assert notebook != 'Untitled', 'Untitled notebook!!!'
    ...
    for srv in notebookapp.list_running_servers():
        try:
            if srv['token']=='' and not srv['password']:  # No token and no password, ahem...
                req = opener.open(srv['url']+'api/sessions')
            else:
                req = opener.open(srv['url']+'api/sessions?token='+srv['token'])
